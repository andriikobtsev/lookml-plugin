// This is a generated file. Not intended for manual editing.
package com.yourcompany.lookml.psi;

import java.util.List;
import org.jetbrains.annotations.*;
import com.intellij.psi.PsiElement;

public interface LookMLDerivedTableBody extends PsiElement {

  @NotNull
  List<LookMLExploreSourceProperty> getExploreSourcePropertyList();

  @NotNull
  List<LookMLIndexesProperty> getIndexesPropertyList();

  @NotNull
  List<LookMLPersistForProperty> getPersistForPropertyList();

  @NotNull
  List<LookMLProperty> getPropertyList();

  @NotNull
  List<LookMLSqlProperty> getSqlPropertyList();

}
